<template>
  <el-dialog :visible="isRequestMedicalInstruction" center @close="closeRequestMedicalInstruction" width="40%">
    ...
    <div>
      ...
      <el-select v-model="mitSelected" size="mini">
        <el-option
          ...
          :key="`mit-${index}`" :value="mit.medicalInstructionTypeId" :label="mit.medicalInstructionTypeName"
        >{{mit.medicalInstructionTypeName}}</el-option>
      </el-select>
    </div>
    <template slot="footer">
      <el-button size="mini" type="primary" @click="sendRequestMedicalInstruction(mitSelected)">Gửi yêu cầu</el-button>
    </template>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('patients', ['sendRequestMedicalInstruction'])
  },
  mounted () {
