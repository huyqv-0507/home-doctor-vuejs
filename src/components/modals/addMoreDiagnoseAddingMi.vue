<template>
  <el-dialog
    :visible="isShowAddMoreDiagnoseAddingMi"
    @close="closeShowAddMoreDiagnoseAddingMi"
    center
...
export default {
  ...
  computed: {
    ...mapState('modals', ['isShowAddMoreDiagnoseAddingMi'])
  },
  mounted () {
  ...
  methods: {
    closeShowAddMoreDiagnoseAddingMi () {
      this.diagnose = ''
      this.$store.dispatch('modals/closeShowAddMoreDiagnoseAddingMi', null, {
        root: true
      })
    },
    ...
    handleSelectDiagnose (diagnose) {
      this.$store.commit('image/addDiagnosesToAddingMi', diagnose, {
        root: true
      })
      this.closeShowAddMoreDiagnoseAddingMi()
    }
  }
}
</script>
